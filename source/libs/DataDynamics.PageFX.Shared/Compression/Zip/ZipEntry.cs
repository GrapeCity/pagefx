...
namespace DataDynamics.Compression.Zip
{
	...
	public class ZipEntry : ICloneable
	{
		...
		internal ZipEntry(string name, int versionRequiredToExtract, int madeByInfo, Stream stream)
		{
			if (name == null)  
			{
				throw new System.ArgumentNullException("ZipEntry name");
		...
		/// </returns>
		/// <see cref="DataDynamics.Compression.Zip.CompressionMethod.Deflated"/>
		/// <see cref="DataDynamics.Compression.Zip.CompressionMethod.Stored"/>
		...
		public Stream Data
		{
			get
			{
				long start = this.offset + ZipConstants.LOCHDR + this.extraDataLen;
				Stream istr = new ZipFile.PartialInputStream(this.stream, start, this.CompressedSize);
			
				CompressionMethod method = this.CompressionMethod;
				switch (method) 
				{
					case CompressionMethod.Stored:
						return istr;
					case CompressionMethod.Deflated:
						return new InflaterInputStream(istr, new DataDynamics.Compression.Zip.Compression.Inflater(true));
					default:
