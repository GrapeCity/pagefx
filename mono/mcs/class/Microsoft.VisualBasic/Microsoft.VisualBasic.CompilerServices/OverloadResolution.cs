    ...
    internal class OverloadResolution
    {
            ...
            if (TargetProcedure.HasParamArray)
            {
                if (!TargetProcedure.ParamArrayExpanded)
                {
                    ...
                    if (!CanPassToParamArray(TargetProcedure, Arguments[length], parameters[index]))
                    {
                        if (flag2)
                        {
                            ReportError(Errors, "ArgumentMismatch3", parameters[index].Name, GetArgumentType(Arguments[length]), parameters[index].ParameterType);
                        }
                        return false;
                    }
                }
        ...
        private static bool CanPassToParamArray(Symbols.Method TargetProcedure, object Argument, ParameterInfo Parameter)
        {
            if (Argument != null)
            {
                ...
                Symbols.Method operatorMethod = null;
                ConversionResolution.ConversionClass class2 = ConversionResolution.ClassifyConversion(parameterType, argumentType, ref operatorMethod);
        ...
        internal static bool CanPassToParameter(Symbols.Method TargetProcedure, object Argument, ParameterInfo Parameter, bool IsExpandedParamArray, bool RejectNarrowingConversions, List<string> Errors, ref bool RequiresNarrowingConversion, ref bool AllNarrowingIsFromObject)
        {
            ...
            bool flag = CanConvert(parameterType, argumentType, RejectNarrowingConversions, Errors, Parameter.Name, false, ref RequiresNarrowingConversion, ref AllNarrowingIsFromObject);
            if (isByRef && flag)
            {
                return CanConvert(argumentType, parameterType, RejectNarrowingConversions, Errors, Parameter.Name, true, ref RequiresNarrowingConversion, ref AllNarrowingIsFromObject);
            }
            return flag;
        }
        ...
        internal static bool InferTypeArgumentsFromArgument(Symbols.Method TargetProcedure, object Argument, ParameterInfo Parameter, bool IsExpandedParamArray, List<string> Errors)
        {
            ...
            if (InferTypeArgumentsFromArgument(GetArgumentType(Argument), parameterType, TargetProcedure.TypeArguments, TargetProcedure.AsMethod(), true))
            {
                return true;
            }
            if (Errors != null)
        ...
        private static bool InferTypeArgumentsFromArgument(Type ArgumentType, Type ParameterType, Type[] TypeInferenceArguments, MethodBase TargetProcedure, bool DigThroughToBasesAndImplements)
        {
            ...
            if (flag || !DigThroughToBasesAndImplements)
            {
                return flag;
            }
        ...
        private static Symbols.Method MoreSpecificProcedure(Symbols.Method Left, Symbols.Method Right, object[] Arguments, string[] ArgumentNames, ComparisonType CompareGenericity, [Optional, DefaultParameterValue(false)] ref bool BothLose, [Optional, DefaultParameterValue(false)] bool ContinueWhenBothLose)
        {
            int num;
            MethodBase base2;
            MethodBase base3;
