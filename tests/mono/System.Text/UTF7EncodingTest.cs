//
// UTF7EncodingTest.cs - NUnit Test Cases for System.Text.Encoding
//
// Authors
...
namespace MonoTests.System.Text
{
    ...
    public class UTF7EncodingTest : Assertion
    {
        ...
        public void TestDirectlyEncoded1()
        {
            ...
            byte[] UTF7Bytes = null;
            Encoding UTF7enc = Encoding.UTF7;

            UTF7Bytes = UTF7enc.GetBytes(UniCodeString);

            AssertEquals("UTF7 #1", 0x61, UTF7Bytes[0]);
            AssertEquals("UTF7 #2", 0x7A, UTF7Bytes[1]);
            AssertEquals("UTF7 #3", 0x41, UTF7Bytes[2]);
            AssertEquals("UTF7 #4", 0x5A, UTF7Bytes[3]);
            AssertEquals("UTF7 #5", 0x30, UTF7Bytes[4]);
            AssertEquals("UTF7 #6", 0x39, UTF7Bytes[5]);
            AssertEquals("UTF7 #7", 0x27, UTF7Bytes[6]);
            AssertEquals("UTF7 #8", 0x3F, UTF7Bytes[7]);
        }
        ...
        public void TestEncodeOptionalEncoded()
        {
            ...
            AssertEquals("UTF7 #1", 0x21, UTF7Bytes[0]);
            AssertEquals("UTF7 #2", 0x26, UTF7Bytes[1]);
            AssertEquals("UTF7 #3", 0x2A, UTF7Bytes[2]);
            AssertEquals("UTF7 #4", 0x3B, UTF7Bytes[3]);

            //Optional characters are not allowed.
            UTF7enc = new UTF7Encoding(false);
            UTF7Bytes = UTF7enc.GetBytes(UniCodeString);

            AssertEquals("UTF7 #5", 0x2B, UTF7Bytes[0]);
            AssertEquals("UTF7 #6", 0x41, UTF7Bytes[1]);
            AssertEquals("UTF7 #7", 0x43, UTF7Bytes[2]);
            AssertEquals("UTF7 #8", 0x45, UTF7Bytes[3]);
            AssertEquals("UTF7 #6", 0x41, UTF7Bytes[1]);
        }
#endif
        ...
        public void TestEncodeUnicodeShifted1()
        {
            ...
            AssertEquals("UTF7 #1", 0x41, UTF7Bytes[0]);
            AssertEquals("UTF7 #2", 0x2B, UTF7Bytes[1]);
            AssertEquals("UTF7 #3", 0x49, UTF7Bytes[2]);
            AssertEquals("UTF7 #4", 0x6D, UTF7Bytes[3]);
            AssertEquals("UTF7 #5", 0x49, UTF7Bytes[4]);
            AssertEquals("UTF7 #6", 0x44, UTF7Bytes[5]);
            AssertEquals("UTF7 #7", 0x6B, UTF7Bytes[6]);
            AssertEquals("UTF7 #8", 0x51, UTF7Bytes[7]);
            AssertEquals("UTF7 #9", 0x2D, UTF7Bytes[8]);
            AssertEquals("UTF7 #10", 0x2E, UTF7Bytes[9]);
        }
        ...
        {
            Encoding UTF7enc = Encoding.UTF7;
            AssertEquals("UTF #1", 50, UTF7enc.GetMaxCharCount(50));
        }
        ...
        public void TestMaxByteCount()
        {
            Encoding UTF7enc = Encoding.UTF7;
#if NET_2_0
                        AssertEquals ("UTF #1", 152, UTF7enc.GetMaxByteCount(50));
#else
            AssertEquals("UTF #1", 136, UTF7enc.GetMaxByteCount(50));
#endif
        }
#endif
        ...
        {
            string s = new Encoding().GetString(
