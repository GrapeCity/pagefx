...
namespace flash.system
{
    ...
    public class Security : Avm.Object
    {
        ...
        /// <summary>
        /// The SWF file is running in an Apollo application, and it was installed with the package (AIR file) for that
        /// application.
        /// </summary>
        [PageFX.AbcClassTrait(16)]
        [PageFX.ABC]
        [PageFX.FP("10.2")]
        public static Avm.String APPLICATION;
        ...
        /// store.example.com.When Security.exactSettings is set to true, Flash Player uses
        /// exact domains for player settings.  When it is set to false, the
        /// Flash Player uses domain settings used in Flash Player 6.  The default
        /// value for exactSettings is false. If you change
        /// exactSettings from its default value, you must do so before any events
        ...
        /// version 7 or later, you must set Security.exactSettings to false
        ...
        /// Security.sandboxType has one of the following values:remote (Security.REMOTE)This SWF file is from an Internet URL and operates under domain-based sandbox
        /// rules.localWithFile (Security.LOCAL_WITH_FILE)This SWF file is a local file, has not been trusted by the user, and
        /// was not published with a networking designation. The SWF file may read from local data sources but may
        ...
        /// read from local data sources.localTrusted (Security.LOCAL_TRUSTED)This SWF file is a local file and has been trusted by the user, using either
        /// the Settings Manager or a FlashPlayerTrust configuration file. The SWF file can read from local data
        /// sources and communicate with the Internet.application (Security.APPLICATION)This SWF file is
        /// running in an Apollo application, and it was installed with the package (AIR file) for that
        /// application. By default, SWF files in the Apollo application sandbox can cross-script any SWF file from any domain
        /// (although SWF files outside of the Apollo application sandbox may not be permitted to cross-script the Apollo SWF file).
        /// By default, SWF files in the Apollo application sandbox can load content and data from any domain. A SWF file of any version may use this property, but is only supported in Flash
        /// Player 8 or later.  Thus you can examine this property, for example,
        /// from a version 7 SWF file playing in Flash Player 8.  This all-versions support means that, if you publish
        /// for a version earlier than 8, you will not know at publish time whether this property will be
        /// supported at playback time. Thus, in a SWF file of version 7 or earlier, this
        /// property has an undefined value; that should happen only when the player version (indicated by
        /// flash.system.Capabilities.version) is earlier than 8.  In that situation, you can determine the sandbox type
        /// according to whether the SWF file is a local file or not. If it is a local file, Flash Player will
        /// classify the SWF file as localTrusted. (Before Flash Player 8, this was how all local content was
        /// treated.) If it is not a local file, Flash Player will classify the SWF file as remote.For more information, see the following:The security chapter in the
        ...
        /// Programming ActionScript 3.0 book and the latest comments on LiveDocs
        /// </summary>
        /// <param name="url">The URL location of the cross-domain policy file to be loaded.</param>
        [PageFX.AbcClassTrait(2)]
        [PageFX.ABC]
        [PageFX.FP9]
        [MethodImpl(MethodImplOptions.InternalCall)]
        public extern static void loadPolicyFile(Avm.String url);
        ...
        [PageFX.FP9]
        [MethodImpl(MethodImplOptions.InternalCall)]
        public extern static void showSettings(Avm.String panel);
